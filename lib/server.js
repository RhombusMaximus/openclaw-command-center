...
var require_notion = __commonJS({
  "src/notion.js"(exports2, module2) {
    var { execFileSync } = require("child_process");
    var path2 = require("path");
    var fs2 = require("fs");
    var configPath = path2.join(__dirname, "../config/notion.json");
    var config = { apiKey: "", projectBoard: null, spaceHQ: null };
    try {
      if (fs2.existsSync(configPath)) {
    ...
    function notionCurl(method, endpoint, body = null, pageSize = 100) {
      ...
      const args2 = ["-s", "-X", method];
      headers.forEach((h) => args2.push("-H", h));
      if (body) {
        args2.push("-d", JSON.stringify(body));
      }
      args2.push(url);
      try {
        const output = execFileSync("curl", args2, { timeout: 15e3 });
...
\`\`\``;
        case "quote":
          return `> ${content.rich_text?.map((t) => t.plain_text).join("") || ""}`;
        case "callout":
          return `\u{1F4A1} ${content.rich_text?.map((t) => t.plain_text).join("") || ""}`;
        case "divider":
          return "\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500";
        case "image":
...
var require_sessions = __commonJS({
  "src/sessions.js"(exports2, module2) {
    ...
    function createSessionsModule2(deps) {
      ...
      // Track pending refresh so callers can await fresh data
      ...
      function getSessionsCached() {
        ...
        if (isStale && !sessionsCache.refreshing) {
          refreshPromise = refreshSessionsCache().then(() => { refreshPromise = null; });
        }
        return sessionsCache.sessions;
      }
      // Returns a promise that resolves only after a fresh refresh
      async function getSessionsCachedAsync() {
        const now = Date.now();
        const isStale = now - sessionsCache.timestamp > SESSIONS_CACHE_TTL;
        if (isStale || !sessionsCache.sessions.length) {
          if (refreshPromise) await refreshPromise;
          else await (refreshPromise = refreshSessionsCache().then(() => { refreshPromise = null; }));
        }
        return sessionsCache.sessions;
      }
    ...
    function getCerebroTopics2(cerebroDir, options = {}) {
      ...
      try {
        ...
        if (!fs2.existsSync(topicsDir)) {
          // cerebro dir exists but not initialized (no topics/ folder yet)
...
var require_tokens = __commonJS({
  "src/tokens.js"(exports2, module2) {
    ...
    // Accurate Claude API pricing (Claude 3.5 Sonnet / Claude 3 Opus)
    ...
    var TOKEN_RATES = {
      ...
      "claude-3-haiku": { input: 0.25, output: 1.25, cacheRead: 0.03, cacheWrite: 0.03 },
      // Nvidia Nemotron (Super, 8B) — uses Sonnet pricing as fallback
      "nemotron": { input: 3, output: 15, cacheRead: 0.3, cacheWrite: 3.75 },
    ...
    function getRatesForModel(model) {
      ...
      if (m.includes("nemotron")) return TOKEN_RATES["nemotron"];
      if (m.includes("opus")) return TOKEN_RATES["claude-3-opus"];
      if (m.includes("haiku")) return TOKEN_RATES["claude-3-haiku"];
      if (m.includes("sonnet")) return TOKEN_RATES["claude-3-5-sonnet"];
      return TOKEN_RATES["default"];
    }
    var tokenUsageCache = { data: null, timestamp: 0, refreshing: false };
    var TOKEN_USAGE_CACHE_TTL = 3e4;
    var refreshInterval = null;
    function emptyUsageBucket(modelKey = null) {
      const base = { input: 0, output: 0, cacheRead: 0, cacheWrite: 0, cost: 0, requests: 0 };
      if (modelKey) return { [modelKey]: { ...base } };
      return base;
    }
    async function refreshTokenUsageAsync2(getOpenClawDir2) {
      ...
      try {
                ...
                for (const line of lines) {
                  ...
                  try {
                    ...
                    if (entry.message?.usage) {
                      ...
                      usage7d.requests++;
                      if (!modelUsage[model]) {
                        modelUsage[model] = { input: 0, output: 0, cacheRead: 0, cacheWrite: 0, cost: 0, requests: 0 };
                      }
                      modelUsage[model].input += input;
                      modelUsage[model].output += output;
                      modelUsage[model].cacheRead += cacheRead;
                      modelUsage[model].cacheWrite += cacheWrite;
        ...
        const result = {
          ...
          // Per-model breakdown for provider cards
          byModel: modelUsage
        };
    ...
    function getModelUsage2(getOpenClawDir2) {
      const usage = getDailyTokenUsage2(getOpenClawDir2);
      return usage?.byModel || {};
    }
    function calculateCostForBucket(bucket, rates = null) {
      // If the bucket has real cost data from API (e.g., Claude API logs), use it
      if (bucket.cost != null && bucket.cost > 0) {
        return {
          inputCost: 0,
          outputCost: 0,
          cacheReadCost: 0,
    ...
    function getTokenStats2(sessions2, capacity, config = {}) {
      ...
      const costs = calculateCostForBucket(usage);
      const isLocalModel = !costs.isRealCost && costs.totalCost === 0;
      const estCost = isLocalModel ? 0 : costs.totalCost;
      const planCost = config?.billing?.claudePlanCost ?? 200;
      const planName = config?.billing?.claudePlanName ?? "Claude Code Max";
      const monthlyApiCost = estCost * 30;
      const monthlySavings = isLocalModel ? 0 : monthlyApiCost - planCost;
      const savingsPositive = !isLocalModel && monthlySavings > 0;
      const sessionCount = sessions2?.length || 1;
      const avgTokensPerSession = Math.round(total / sessionCount);
      const avgCostPerSession = sessionCount > 0 ? estCost / sessionCount : 0;
      const windowConfigs = {
        "24h": { days: 1, label: "24h" },
      ...
      for (const [key, windowConfig] of Object.entries(windowConfigs)) {
        ...
        const isWindowLocal = bucket.cost == null || bucket.cost === 0;
        savingsWindows[key] = {
          label: windowConfig.label,
          estCost: isWindowLocal ? "Local" : `$${formatNumber(dailyAvg)}`,
          estMonthlyCost: isWindowLocal ? "Local" : `$${Math.round(monthlyProjected).toLocaleString()}`,
          estSavings: isWindowLocal ? null : (windowSavingsPositive ? `$${formatNumber(windowSavings)}/mo` : null),
          savingsPercent: isWindowLocal ? 0 : (windowSavingsPositive ? Math.round(windowSavings / monthlyProjected * 100) : 0),
          requests: bucket.requests,
          isLocal: isWindowLocal
        };
      }
      return {
        ...
        mainLimit,
        subagentLimit,
        estCost: isLocalModel ? "Local" : `$${formatNumber(estCost)}`,
        ...
        estMonthlyCost: isLocalModel ? "Local" : `$${Math.round(monthlyApiCost).toLocaleString()}`,
        // Multi-window savings (24h, 3da, 7da)
        savingsWindows,
        // Per-session averages
        avgTokensPerSession: formatTokens(avgTokensPerSession),
        avgCostPerSession: isLocalModel ? "Local" : `$${avgCostPerSession.toFixed(2)}`,
        sessionCount,
        isLocalModel
      };
    }
  ...
  "src/state.js"(exports2, module2) {
    ...
    function createStateModule2(deps) {
      ...
      function getFullState() {
        ...
        let modelUsage = {};
        try {
          modelUsage = getModelUsage2();
        } catch (e) {
          console.error("[State] modelUsage:", e.message);
        }
        try {
...
var state = createStateModule({
  ...
  getSessionsCachedAsync: () => sessions.getSessionsCachedAsync(),
  getSystemVitals,
  ...
  getModelUsage: () => getModelUsage(getOpenClawDir),
  getTokenStats,
...
var server = http.createServer((req, res) => {
  ...
  if (pathname === "/api/status") {
    ...
    if (method === "POST") {
      ...
      req.on("end", () => {
        try {
          const { action, path: openPath } = JSON.parse(body || "{}");
          if (action === "open-path" && openPath) {
            const { execSync } = require("child_process");
            const os = require("os");
            const resolved = openPath.replace("~", os.homedir());
            try {
              if (process.platform === "darwin") execSync(`open "${resolved}"`);
              else execSync(`xdg-open "${resolved}"`);
              res.writeHead(200, { "Content-Type": "application/json" });
              res.end(JSON.stringify({ success: true, output: "Opened: " + resolved }));
            } catch (e) {
              res.writeHead(200, { "Content-Type": "application/json" });
              res.end(JSON.stringify({ success: false, error: "Could not open: " + e.message }));
            }
          } else {
            res.writeHead(400, { "Content-Type": "application/json" });
            res.end(JSON.stringify({ error: "Unknown POST action or missing path" }));
          }
        } catch (e) {
          res.writeHead(400, { "Content-Type": "application/json" });
    ...
    // Force cache warm-up before sending first state to avoid empty data
    (async () => {
      try {
        const freshSessions = await sessions.getSessionsCachedAsync();
        const cachedState = state.getFullState();
        if (cachedState && freshSessions.length > 0) {
          sendSSE(res, "update", cachedState);
        } else {
          sendSSE(res, "update", { sessions: freshSessions, loading: false });
        }
      } catch (e) {
        sendSSE(res, "update", { sessions: [], loading: false });
      }
    })();
    req.on("close", () => {
    ...
    const events = [
      ...
      ...projects.filter((p) => p.dueDate && !p.archived).map((p) => ({
        date: p.dueDate,
        title: p.name,
        type: "project",
        color: "#22c55e"
