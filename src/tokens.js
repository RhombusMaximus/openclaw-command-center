...
// Create empty usage bucket (optionally model-keyed)
...
// Get rate for a given model — checks config.billing.modelRates if provided
function getRatesForModel(model, modelRates) {
  if (!modelRates) return TOKEN_RATES;
  if (modelRates[model]) return modelRates[model];
  for (const [key, rates] of Object.entries(modelRates)) {
...
const modelTokenCache = { data: null, timestamp: 0, refreshing: false };
const MODEL_TOKEN_CACHE_TTL = 30000;
...
async function refreshTokenUsageAsync(getOpenClawDir) {
  ...
  try {
    ...
    // Per-model usage for provider cards (model name → bucket)
    const modelUsage = {};
            ...
            for (const line of lines) {
              ...
              try {
                ...
                if (entry.message?.usage) {
                  ...
                  // Per-model tracking (add to 7d window for simplicity)
                  if (!modelUsage[model]) {
...
// Get per-model token usage for provider cards
function getModelUsage(getOpenClawDir) {
  const usage = getDailyTokenUsage(getOpenClawDir);
  return usage?.byModel || {};
}

// Calculate cost for a usage bucket
// If bucket has pre-stored cost (from JSONL), use it; otherwise compute from rates
function calculateCostForBucket(bucket, rates = TOKEN_RATES) {
  // Use stored cost if available (from actual model provider billing)
